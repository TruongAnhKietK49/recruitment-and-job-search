...
function setClientCache(data) {
  try {
    localStorage.setItem(getCacheKey(), JSON.stringify({ data, expiredAt: Date.now() + CLIENT_CACHE_TTL }));
  } catch {}
}
...
function renderAiCards(recommendations = [], container, loading) {
    ...
    container.innerHTML = `
      <div class="col-12 text-center py-4">
        <div class="alert alert-warning d-inline-block shadow-sm" style="border-radius:15px;">
          ...
          AI chưa tìm thấy việc phù hợp. Hãy cập nhật thêm kỹ năng nhé!
        </div>
      </div>
    `;
    return;
  ...
  container.innerHTML = recommendations
    .map((item) => {
      ...
      const displayJobType = jobTypeMap[job.jobType] || job.jobType || "Chưa cập nhật";
      const matchScore = item.matchScore || "Phù hợp";
      const aiReason = item.aiReason || item.reason || "Công việc này phù hợp với hồ sơ và kỹ năng của bạn.";

      return `
      <div class="col-lg-4 col-md-6 mb-4">
        <div
          class="job-card h-100 p-4 border rounded-4 shadow-sm position-relative bg-white d-flex flex-column"
        ...
        >
          ...
          <div class="d-flex align-items-center mb-3 pe-5">
            <img
              src="${companyLogo}"
              alt="${companyName}"
              class="rounded border p-1 me-3 bg-white flex-shrink-0"
              style="width: 60px; height: 60px; object-fit: contain;"
            >

            <div style="min-width: 0;">
              <h5 class="job-title text-truncate mb-1 fw-bold text-dark" title="${job.title || ""}">
                ${job.title || "Chưa cập nhật tiêu đề"}
              </h5>
              <div class="company-name text-muted small text-truncate" title="${companyName}">
                ${companyName}
              </div>
            </div>
          </div>
          ...
          <div class="job-info text-muted small mb-3">
            <div class="mb-2 text-truncate" title="${jobLocation}">
              <i class="bi bi-geo-alt text-primary me-2"></i>${jobLocation}
            </div>
            <div class="mb-2">
            ...
            <div class="mb-2">
              <i class="bi bi-clock text-primary me-2"></i>${displayJobType}
            </div>
          </div>

          <div class="alert alert-info small rounded-4 py-2 px-3 mb-3">
...
async function loadAiRecommendations(isForceReload = false) {
  ...
  if (!isForceReload) {
    ...
    if (cached?.length > 0) {
      renderAiCards(cached, container, loading);
      return;
    }
  }
    ...
    const errText =
      msg.includes("quota") || msg.includes("Quota")
        ...
        : msg.includes("timeout")
          ? "AI phân tích quá lâu. Vui lòng thử lại!"
...
async function initHomePage() {
  ...
  await Promise.all([refreshJobsView(), loadAiRecommendations()]);
}
...
function getPaginationItems(page, total) {
  ...
  const pages = [...visiblePages].filter((item) => item >= 1 && item <= safeTotal).sort((a, b) => a - b);
